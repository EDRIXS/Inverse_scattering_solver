__all__ = ['saveops','ed_1v1c_init','ed_1v1c_py_full','rixs_1v1c_py']
...
def ed_1v1c_py_full(shell_name, sptab2, sptab4, sptabtr,*, shell_level=None, v_soc=None, c_soc=0,
               v_noccu=1, slater=None, ext_B=None,
               tenDq=0 ):
...
#    eval_n, evec_n = jnp.linalg.eigh(hmat_n)
    print("edrixs >>> Done !")
#    t=time.time()
...
def cb_op2(oper_O, TL, TR):
    oper_O = jnp.array(oper_O, order='K')
    dim = oper_O.shape
    if len(dim) < 2:
        raise Exception("Dimension of oper_O should be at least 2")
    elif len(dim) == 2:
        res = jnp.dot(jnp.dot(jnp.conj(jnp.transpose(TL)), oper_O), TR)
    else:
        tot = jnp.prod(dim[0:-2])
        tmp_oper = oper_O.reshape((tot, dim[-2], dim[-1]))
        for i in range(tot):
            tmp_oper[i] = jnp.dot(jnp.dot(jnp.conj(jnp.transpose(TL)), tmp_oper[i]), TR)
